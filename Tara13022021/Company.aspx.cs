...
public partial class Company : System.Web.UI.Page
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            TimeZoneInfo tzi = TimeZoneInfo.FindSystemTimeZoneById("India Standard Time");
            DateTime localTime = TimeZoneInfo.ConvertTimeFromUtc(utcTime, tzi);
            dt1 = bal.checkcompanynamemasterBAL(txtName.Text);
            if (dt1.Rows.Count > 0)
            {
                //mpbtype.Show();

               ShowMessage("Name Already Exist!!!", MessageType.Error);
                //lblmsg5.Text = "Name Already Exist!!!";
            }
            else
            {


                bal.tbl_Company_Master_InsertBAL(lblcomno.Text, txtName.Text, "", txtaddress.Text, txtcity.Text, txtstate.Text, txtdistrict.Text, txtcountry.Text, txtpincode.Text, txtphno.Text, txtemail.Text, Convert.ToInt32(dpbusstype.SelectedValue.ToString()), Convert.ToInt32(dpindustry.SelectedValue.ToString()), txturl.Text, rbtnstatus.SelectedItem.Text, txtgstno.Text, txtbankname.Text, txtaccno.Text, txtifsccode.Text, lblloginid.Text, localTime, "", "", "", "", "");
    ...
    protected void btnaddCompanyImage_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (fuCompanyImage.HasFile)
            {
                ...
                fuCompanyImage.SaveAs(Server.MapPath("~/CompanyLogo/") + randomfilename + extention);
                string filepath = "~/CompanyLogo/" + randomfilename + extention;
