        ...
        if (!IsPostBack)
        {
            try
            {
                ...
                else
                {
                    ...
                    dpfollowstatus.SelectedIndex = 2;
                    //bindTaxationdata();
                }
            }
            catch (Exception ex)
    ...
    protected void grdTerms_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName == "editterms")
            {
                ...
                //int gr = index;
                Response.Write("<script>window.open ('UpdateTerms.aspx?no=" + index + "','_blank');</script>");
                ...
                try
                {
                    foreach (GridViewRow row in grddata1.Rows)
                    {
                        if (row.RowType == DataControlRowType.DataRow)
                        {
                            ...
                            string series = ((Label)row.FindControl("lbltermsseries")).Text;
                            
                            if (chkrow.Checked)
                            {
                                bal.tbl_Quotation_tandc_InsertBAL(Convert.ToInt32(lblcomno.Text), id, title, descp, "True", lblloginid.Text, localTime, series, "", "", "", "");
                            }
                            else
                            {
                                bal.tbl_Quotation_tandc_InsertBAL(Convert.ToInt32(lblcomno.Text), id, title, descp, "False", lblloginid.Text, localTime, series, "", "", "", "");
                            }
                        }
                    }
                }
